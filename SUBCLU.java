...
public class SUBCLU<V extends NumberVector> extends AbstractAlgorithm<Clustering<SubspaceModel>> implements SubspaceClusteringAlgorithm<SubspaceModel> {
  ...
  public Clustering<SubspaceModel> run(Relation<V> relation) {
    ...
    //thesis
    String candidateStringDimList = ""; 
    boolean validSubspace = false; //variable contains Y/N if currentSubspace fulfills dimensionRequirements 
    List<String> subspaceDimensionList = new ArrayList<String>();
    List<String> testList = Arrays.asList("1");
    /////////
    ...
    for(int d = 0; d < dimensionality; d++) {
      ...
      //this line parses the dimension list of the subspace
      candidateStringDimList= currentSubspace.dimensonsToString();
      candidateStringDimList = candidateStringDimList.replace(" ","").replace("[", "").replaceAll("]", "");
      subspaceDimensionList = new ArrayList<String>(Arrays.asList(candidateStringDimList.split(","))); //convert to Arraylist of stirngs
      
      //check if subspace is valid
      validSubspace = subspaceContainsAllDimensions(subspaceDimensionList, testList);
      ...
      //add code here to exclude subspace and cluster into S_1 and clusterMap
      if(!clusters.isEmpty()/* && validSubspace*/) {
    ...
    for(int d = 0; d < dimensionality - 1; d++) {
      ...
      stepprog.beginStep(d + 2, "Calling Generate SubspaceCandidates for " + (d + 2 ) + "for : " + subspaces.size() + "of subspaces" , LOG);
      List<Subspace> candidates = generateSubspaceCandidates(subspaces);
      stepprog.beginStep(d + 2, "Exit Generate SubspaceCandidates for " + (d + 2 ) + "for : " + subspaces.size() + "of subspaces, " +  candidates.size() + " candidates", LOG);
      List<Subspace> s_d = new ArrayList<>();
      //thesis: remove non-valid from candidates
      //In order to improve the runtime of DBSCAN, only the points known to belong to clusters in one 
      //{\displaystyle k} k-dimensional subspace (which is chosen to contain as little clusters as possible) are considered. 
      //Due to the downward-closure property, other point cannot be part of a {\displaystyle k+1} k+1-dimensional cluster anyway.
      for(Subspace candidate : candidates) {
        
        stepprog.beginStep(d+2, "Search of best subspace started for candidate :" + candidate.dimensonsToString(), LOG);
        Subspace bestSubspace = bestSubspace(subspaces, candidate, clusterMap);
        stepprog.beginStep(d+2, "Search of best subspace completed : " + bestSubspace.dimensonsToString(), LOG);
        ...
        for(Cluster<Model> cluster : bestSubspaceClusters) {
          stepprog.beginStep(d+2, "Started Running DBScan for : " + bestSubspace.dimensonsToString(), LOG);
          List<Cluster<Model>> candidateClusters = runDBSCAN(relation, cluster.getIDs(), candidate);
          stepprog.beginStep(d+2, "Completed Running DBScan for : " + bestSubspace.dimensonsToString() + ", " , LOG);
    ...
    for(int i = 0; i < subspaces.size(); i++) {
      ...
      for(int j = i + 1; j < subspaces.size(); j++) {
        ...
        if(candidate != null) {
          ...
          //this line parses the dimension list of the subspace
          candidateStringDimList= candidate.dimensonsToString();
          candidateStringDimList = candidateStringDimList.replace(" ","").replace("[", "").replaceAll("]", "");
          subspaceDimensionList = new ArrayList<String>(Arrays.asList(candidateStringDimList.split(","))); //convert to Arraylist of stirngs
          ...
          boolean irrelevantCandidate = true;
          for(Subspace s : lowerSubspaces) {
            //la inclusion de esta metrica va a generar una explosion de subespacios a evaluar (buscar los clusters en el hash de mapeo, pero no correra dbscan para todos)  
            //if(!subspaces.contains(s) && validSubspace /*// Esta generando cantidades de combinaciones infinitas */) {
            if(subspaces.contains(s) && validSubspace /*// Esta generando cantidades de combinaciones infinitas */) {
              irrelevantCandidate = false;//true
              break;
            }
          }
          if(!irrelevantCandidate){/* && validSubspace) {*/
...
//this method checks the subspace and validates if it contains all user requested dimensions
  //in other words this checks if subspace qualifies according to user criteria
  public static boolean subspaceContainsAllDimensions(List<String> subspaceList, List<String> requiredDimensions)
  {
    //21/01/2017 Since subspaces are built bottom up from 1 dim to N dim, we shouldnt discard subspaces if it doesnt contains 
    //complete list of variables, for this, requirement is that it every candidate subspace should contain at least ONE variable
    //from request list.
    
    
    boolean containsDimensions = false;

    for(String userRequestedDimension: requiredDimensions) {
      if(subspaceList.contains(userRequestedDimension))
      {
         return true;//returns false if at least one dimension doesnt exist in subspace
      }   
      /*if(!subspaceList.contains(userRequestedDimension))
         {
            return false;//returns false if at least one dimension doesnt exist in subspace
         }*/
   ...
   *         cluster
   */
  //thesis: puedo modificar el bestSubspace ? modificar para que no retorne ninguno q no cumpla el criterio minimo de agrupamiento
...
//mejor subespacio es seleccionado en base al tama;o del cluster mas pequeño del subespacio, entre mas pequeño el cluster mejor el subespacio
